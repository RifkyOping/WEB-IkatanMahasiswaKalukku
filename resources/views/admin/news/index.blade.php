    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="flex justify-between items-center mb-8 px-2">
                <div>
                    <h3 class="text-xl font-bold text-gray-800">Daftar Berita</h3>
                    <p class="text-gray-500 text-sm mt-1">Kelola semua artikel berita yang tampil di website.</p>
                </div>
                <a href="{{ route('admin.news.create') }}" class="flex items-center gap-2 px-6 py-3 bg-imk-600 text-white font-bold rounded-full hover:bg-imk-700 hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5">
                    ...
                    <table class="min-w-full w-full border-collapse">
                        <thead>
                            <tr class="border-b-2 border-gray-100">
                                ...
                                <th class="py-4 px-6 text-left text-sm font-bold text-gray-400 uppercase tracking-wider">Judul</th>
                                <th class="py-4 px-6 text-left text-sm font-bold text-gray-400 uppercase tracking-wider">Tanggal</th>
                        ...
                        <tbody class="divide-y divide-gray-50">
                            @forelse($news as $item)
                                <tr class="hover:bg-gray-50/50 transition duration-200">
                                    ...
                                    <td class="py-5 px-6 text-left">
                                        <div class="flex items-center gap-4">
                                            @if($item->images && is_array($item->images) && count($item->images) > 0)
                                                <img src="{{ asset('storage/' . $item->images[0]) }}" class="w-14 h-14 rounded-2xl object-cover shadow-sm">
                                            @else
                                                <div class="w-14 h-14 rounded-2xl bg-gray-100 flex items-center justify-center text-gray-400 shadow-sm">
                                    ...
                                    <td class="py-5 px-6 text-left text-gray-500 font-medium">{{ $item->created_at->format('d M Y') }}</td>
                                    <td class="py-5 px-6 text-center">
                                        <div class="flex items-center justify-center gap-3">
                                            <a href="{{ route('admin.news.edit', $item->id) }}" class="p-2.5 text-blue-500 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 rounded-xl transition shadow-sm">
                                ...
                                <tr>
                                    <td colspan="4" class="py-16 text-center">
                                        <div class="flex flex-col items-center justify-center text-gray-400">
                                            ...
                                            <p class="text-lg">Belum ada berita yang ditambahkan.</p>
                                        </div>
                                    </td>
                                </tr>
                            @endforelse
