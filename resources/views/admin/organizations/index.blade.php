    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="flex justify-between items-center mb-6">
                <div>
                    <h3 class="text-xl font-bold text-gray-800">Daftar Jabatan & Divisi</h3>
                    <p class="text-gray-500 text-sm">Kelola kotak jabatan di bagan struktur organisasi.</p>
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full w-full">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="py-4 px-6 text-left font-bold text-gray-500">Urutan</th>
                                <th class="py-4 px-6 text-left font-bold text-gray-500">Nama Jabatan/Divisi</th>
                                <th class="py-4 px-6 text-left font-bold text-gray-500">Bagian (Section)</th>
                                <th class="py-4 px-6 text-center font-bold text-gray-500">Anggota</th>
                                <th class="py-4 px-6 text-center font-bold text-gray-500">Aksi</th>
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100">
                            @foreach($organizations as $org)
                                <tr class="hover:bg-gray-50">
                                    <td class="py-4 px-6">{{ $org->sort_order }}</td>
                                    <td class="py-4 px-6 font-bold text-[#051F20]">{{ $org->name }}</td>
                                    <td class="py-4 px-6">
                                        <span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-xs font-bold uppercase">{{ $org->section }}</span>
                                    </td>
                                    <td class="py-4 px-6 text-center">
                                        <a href="{{ route('admin.members.index', $org->id) }}" class="inline-flex items-center gap-2 px-4 py-2 bg-imk-50 text-imk-600 hover:bg-imk-100 font-bold rounded-xl transition">
                                            Kelola Anggota ({{ $org->members()->count() }})
                                        </a>
                                    </td>
                                    <td class="py-4 px-6 text-center">
                                        <div class="flex items-center justify-center gap-3">
                                            <a href="{{ route('admin.organizations.edit', $org->id) }}" class="p-2.5 text-blue-500 bg-blue-50 hover:bg-blue-100 hover:text-blue-700 rounded-xl transition shadow-sm">
                                                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                                            </a>
                                            <form action="{{ route('admin.organizations.destroy', $org->id) }}" method="POST" class="inline-block delete-form">
                                                @csrf
                                                @method('DELETE')
            ...
            <div class="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-[30px] shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                <form action="{{ route('admin.organizations.store') }}" method="POST">
                    ...
                    <div class="px-8 pt-8 pb-6">
                        <h3 class="text-2xl font-bold text-gray-900 mb-6">Tambah Jabatan / Divisi</h3>
                        
                        <div class="mb-4">
                            <label class="block text-sm font-bold text-gray-700 mb-2">Nama Jabatan / Divisi</label>
                            <input type="text" name="name" required class="w-full rounded-xl border-gray-300 focus:border-imk-500 focus:ring focus:ring-imk-200" placeholder="Contoh: Divisi Keagamaan">
                        </div>
