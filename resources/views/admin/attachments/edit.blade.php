    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-[39px] shadow-xl p-10 md:p-12 relative overflow-hidden">
                <form action="{{ route('admin.attachments.update', $attachment->id) }}" method="POST" enctype="multipart/form-data" class="space-y-8 relative z-10">
                    @csrf
                    @method('PUT')

                    <div>
                        <label for="title" class="block text-sm font-bold text-gray-700 mb-2">Judul / Nama Tampilan</label>
                        <input type="text" name="title" id="title" value="{{ old('title', $attachment->title) }}" required class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors">
                        @error('title')
                    ...
                    <div>
                        ...
                        <div class="p-4 bg-gray-50 rounded-2xl border border-gray-200 mb-4 flex items-center justify-between">
                            <span class="text-gray-600 font-medium">{{ $attachment->original_name ?? 'File tidak diketahui' }}</span>
                            <a href="{{ route('lampiran.download', $attachment->id) }}" class="text-imk-600 hover:text-imk-800 font-bold text-sm underline">Download</a>
                        </div>

                        <label for="file" class="block text-sm font-bold text-gray-700 mb-2">Ganti File Lampiran (Opsional)</label>
                        <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-2xl bg-gray-50 hover:bg-gray-100 transition-colors relative group">
                            <div class="space-y-1 text-center">
                                ...
                                <p class="text-xs text-gray-500 mt-2">Biarkan kosong jika tidak ingin mengubah file.</p>
                            </div>
                        </div>
                        @error('file')
                            <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> {{ $message }}</p>
                    ...
                    <div class="pt-4 flex items-center gap-4 border-t border-gray-100">
                        <button type="submit" class="px-8 py-3 bg-imk-600 text-white font-bold rounded-full hover:bg-imk-700 shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5">Perbarui Lampiran</button>
