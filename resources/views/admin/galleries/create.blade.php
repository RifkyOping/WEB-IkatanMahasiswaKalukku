    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white rounded-[39px] shadow-xl p-10 md:p-12 relative overflow-hidden">
                <form action="{{ route('admin.galleries.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8 relative z-10">
                    @csrf

                    <div>
                        <label for="title" class="block text-sm font-bold text-gray-700 mb-2">Judul Kegiatan</label>
                        <input type="text" name="title" id="title" value="{{ old('title') }}" required class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors" placeholder="Masukkan judul acara atau kegiatan">
                        @error('title')
                    ...
                    <div>
                        <label for="content" class="block text-sm font-bold text-gray-700 mb-2">Deskripsi Singkat</label>
                        <textarea name="content" id="content" rows="4" required class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors" placeholder="Berikan penjelasan singkat mengenai momen ini...">{{ old('content') }}</textarea>
                        @error('content')
                            <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> {{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="date" name="date" id="date" value="{{ old('date') }}" required class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors">
                            @error('date')
                                <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> {{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="date" name="end_date" id="end_date" value="{{ old('end_date') }}" class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors">
                            @error('end_date')
                                <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> {{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <input type="url" name="drive_link" id="drive_link" value="{{ old('drive_link') }}" class="block w-full border-gray-200 focus:border-imk-400 focus:ring-imk-400 rounded-2xl shadow-sm text-gray-700 p-4 bg-gray-50 hover:bg-white transition-colors" placeholder="https://drive.google.com/...">
                        @error('drive_link')
                            <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> {{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            <div class="space-y-1 text-center">
                                ...
                                <div class="flex text-sm text-gray-600 justify-center">
                                    <label for="images" class="relative cursor-pointer bg-white rounded-md font-medium text-imk-600 hover:text-imk-500 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-imk-500 p-1 px-2">
                                        ...
                                        <input id="images" name="images[]" type="file" multiple accept="image/*" required class="sr-only">
                                    </label>
                                    <p class="pl-1 pt-1">atau drag and drop</p>
                                </div>
                                <p class="text-xs text-gray-500 mt-2">Bisa memilih lebih dari satu file (Max 3).<br>PNG, JPG up to 2MB per file.</p>
                            </div>
                        </div>
                        @error('images')
                            <p class="text-red-500 text-sm mt-2 flex items-center gap-1"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg> {{ $message }}</p>
                    ...
                    <div class="pt-4 flex items-center gap-4 border-t border-gray-100">
                        <button type="submit" class="px-8 py-3 bg-imk-600 text-white font-bold rounded-full hover:bg-imk-700 shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-0.5">Simpan Galeri</button>
